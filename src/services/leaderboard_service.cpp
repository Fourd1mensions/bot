...
LeaderboardService::LeaderboardService(Request& request, BeatmapDownloader& beatmap_downloader,
                                       ...
                                       dpp::cluster&              bot) :
    request_(request), beatmap_downloader_(beatmap_downloader),
    chat_context_service_(chat_context_service),
    beatmap_resolver_service_(beatmap_resolver_service),
    user_mapping_service_(user_mapping_service), user_resolver_service_(user_resolver_service),
    message_presenter_(message_presenter), performance_service_(performance_service), bot_(bot) {}

dpp::message LeaderboardService::build_page(const LeaderboardState& state,
  ...
  if (osu_file_path_opt) {
    ...
    if (diff_opt) {
      ...
      spdlog::debug("[LB] Got beatmap data from performance service: AR={:.2f} OD={:.2f}",
  ...
  footer_values["filter"] =
      active_mods.empty() ? "" : fmt::format(" \xe2\x80\xa2 Filter: +{}", active_mods);
  footer_values["sort"] = (state.sort_method != LbSortMethod::PP) ?
      fmt::format(" \xe2\x80\xa2 sorted by {}", sort_method_str) :
      "";
  footer_values["caller_rank"] =
      caller_rank_num.empty() ? "" : fmt::format(" \xe2\x80\xa2 your rank: #{}", caller_rank_num);
  ...
  for (size_t i = start; i < end; i++) {
    ...
    if (display_pp <= 0.01 && osu_file_path_opt.has_value() && score.get_mode() == "osu") {
      ...
      if (perf_opt.has_value()) {
        ...
        spdlog::info("[LB] PP calc for {}: {:.2f}pp (lazer={})", score.get_user_id(), display_pp,
                     params.lazer);
      }
    }
  ...
  return message_presenter_.build_leaderboard_page(state.beatmap, scores_on_page, footer_values,
                                                   active_mods, state.total_pages,
...
void LeaderboardService::remove_message_components(dpp::snowflake channel_id,
...
void LeaderboardService::schedule_button_removal(dpp::snowflake       channel_id,
                                                 dpp::snowflake       message_id,
