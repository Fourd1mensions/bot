...
std::unique_ptr<pqxx::connection> ConnectionPool::acquire() {
  ...
  if (!conn->is_open()) {
    ...
    {
      std::lock_guard<std::mutex> al(alert_mutex_);
      auto                        now = std::chrono::steady_clock::now();
      if (alert_cb_ && (now - last_alert_ > alert_cooldown_)) {
        last_alert_ = now;
        auto cb     = alert_cb_;
        lock.unlock();
        cb("Database connection lost, attempting reconnect...");
        lock.lock();
      }
    }
    ...
    try {
      ...
      spdlog::info("[DB] Reconnected successfully");
    } catch (const std::exception& e) {
      spdlog::error("[DB] Reconnect failed: {}", e.what());
      throw;
    }
  }
...
void ConnectionPool::set_alert_callback(AlertCallback cb, std::chrono::seconds cooldown) {
...
void Database::init(const std::string& host, int port, const std::string& dbname,
  ...
  std::string conn_str = std::format("host={} port={} dbname={} user={} password={}", host, port,
...
void Database::set_alert_callback(ConnectionPool::AlertCallback cb, std::chrono::seconds cooldown) {
  pool_->set_alert_callback(std::move(cb), cooldown);
}
...
void Database::set_user_mapping(dpp::snowflake discord_id, int64_t osu_user_id, bool is_oauth) {
  execute([&](pqxx::connection& conn) {
    ...
    txn.exec_params(
        ...
        "ON CONFLICT (discord_id) DO UPDATE SET osu_user_id = $2, is_oauth_linked = $3, updated_at "
        "= CURRENT_TIMESTAMP",
...
std::optional<int64_t> Database::get_osu_user_id(dpp::snowflake discord_id) {
  return execute([&](pqxx::connection& conn) -> std::optional<int64_t> {
    pqxx::work txn(conn);
    auto       result = txn.exec_params("SELECT osu_user_id FROM users WHERE discord_id = $1",
...
bool Database::remove_user_mapping(dpp::snowflake discord_id) {
  return execute([&](pqxx::connection& conn) {
    pqxx::work txn(conn);
    auto       result = txn.exec_params("DELETE FROM users WHERE discord_id = $1",
                                        static_cast<int64_t>(discord_id));
    txn.commit();
...
bool Database::is_user_oauth_linked(dpp::snowflake discord_id) {
  return execute([&](pqxx::connection& conn) -> bool {
    pqxx::work txn(conn);
    auto       result = txn.exec_params("SELECT is_oauth_linked FROM users WHERE discord_id = $1",
                                        static_cast<int64_t>(discord_id));
...
std::vector<std::pair<dpp::snowflake, int64_t>> Database::get_all_user_mappings() {
  return execute([&](pqxx::connection& conn) {
    ...
    for (const auto& row : result) {
      mappings.emplace_back(dpp::snowflake(row[0].as<uint64_t>()), row[1].as<int64_t>());
    }
...
void Database::set_chat_context(dpp::snowflake channel_id, dpp::snowflake message_id,
  ...
  execute([&](pqxx::connection& conn) {
    pqxx::work txn(conn);
    txn.exec_params("INSERT INTO chat_map (channel_id, message_id, beatmap_id) VALUES ($1, $2, $3) "
                    "ON CONFLICT (channel_id) DO UPDATE SET message_id = $2, beatmap_id = $3, "
...
std::optional<std::pair<dpp::snowflake, std::string>>
Database::get_chat_context(dpp::snowflake channel_id) {
  return execute(
      [&](pqxx::connection& conn) -> std::optional<std::pair<dpp::snowflake, std::string>> {
        pqxx::work txn(conn);
        auto       result =
            txn.exec_params("SELECT message_id, beatmap_id FROM chat_map WHERE channel_id = $1",
        ...
        return std::make_pair(dpp::snowflake(result[0][0].as<uint64_t>()),
...
bool Database::remove_chat_context(dpp::snowflake channel_id) {
  return execute([&](pqxx::connection& conn) {
    pqxx::work txn(conn);
    auto       result = txn.exec_params("DELETE FROM chat_map WHERE channel_id = $1",
                                        static_cast<int64_t>(channel_id));
    txn.commit();
    return result.affected_rows() > 0;
  });
...
void Database::register_beatmap_file(int64_t beatmapset_id, const std::string& osz_path,
  ...
  execute([&](pqxx::connection& conn) {
    pqxx::work txn(conn);
    txn.exec_params("INSERT INTO beatmap_files (beatmapset_id, osz_path, mirror_hostname, "
                    "file_size, last_accessed, access_count) "
                    "VALUES ($1, $2, $3, $4, CURRENT_TIMESTAMP, 0) "
                    "ON CONFLICT (beatmapset_id) DO UPDATE SET "
                    "osz_path = $2, mirror_hostname = $3, file_size = $4, last_accessed = "
                    "CURRENT_TIMESTAMP, updated_at = CURRENT_TIMESTAMP",
                    beatmapset_id, osz_path,
                    mirror_hostname ? pqxx::zview(*mirror_hostname) : pqxx::zview(), file_size);
    txn.commit();
...
void Database::update_file_access(int64_t beatmapset_id) {
  execute([&](pqxx::connection& conn) {
    pqxx::work txn(conn);
    txn.exec_params("UPDATE beatmap_files SET last_accessed = CURRENT_TIMESTAMP, "
...
std::optional<BeatmapFile> Database::get_beatmap_file(int64_t beatmapset_id) {
  return execute([&](pqxx::connection& conn) -> std::optional<BeatmapFile> {
    ...
    file.mirror_hostname =
        row[2].is_null() ? std::nullopt : std::optional(row[2].as<std::string>());
...
std::vector<BeatmapFile> Database::get_all_beatmap_files() {
  return execute([&](pqxx::connection& conn) {
    pqxx::work               txn(conn);
    auto                     result = txn.exec("SELECT beatmapset_id, osz_path, mirror_hostname "
                                                                   "FROM beatmap_files");
...
bool Database::beatmap_file_exists(int64_t beatmapset_id) {
  return execute([&](pqxx::connection& conn) {
    pqxx::work txn(conn);
    auto       result =
        txn.exec_params("SELECT 1 FROM beatmap_files WHERE beatmapset_id = $1", beatmapset_id);
    return !result.empty();
  });
}
...
bool Database::remove_beatmap_file(int64_t beatmapset_id) {
  return execute([&](pqxx::connection& conn) {
    pqxx::work txn(conn);
    auto       result =
        txn.exec_params("DELETE FROM beatmap_files WHERE beatmapset_id = $1", beatmapset_id);
    txn.commit();
    return result.affected_rows() > 0;
  });
}
...
std::string Database::create_beatmap_extract(int64_t beatmapset_id, const std::string& extract_path,
  ...
  return execute([&](pqxx::connection& conn) {
    ...
    spdlog::debug("Created beatmap extract {} for set {} (TTL: {}h)", extract_id, beatmapset_id,
                  ttl.count());
    return extract_id;
  });
}
...
std::optional<BeatmapExtract> Database::get_beatmap_extract(const std::string& extract_id) {
  return execute([&](pqxx::connection& conn) -> std::optional<BeatmapExtract> {
    pqxx::work txn(conn);
    auto       result =
        txn.exec_params("SELECT extract_id, beatmapset_id, extract_path, created_at, expires_at "
...
bool Database::remove_beatmap_extract(const std::string& extract_id) {
  return execute([&](pqxx::connection& conn) {
    pqxx::work txn(conn);
    auto result = txn.exec_params("DELETE FROM beatmap_extracts WHERE extract_id = $1", extract_id);
    txn.commit();
    return result.affected_rows() > 0;
  });
}
...
std::vector<BeatmapExtract> Database::cleanup_expired_extracts() {
  return execute([&](pqxx::connection& conn) {
    pqxx::work txn(conn);
    auto       result =
        txn.exec("DELETE FROM beatmap_extracts WHERE expires_at < CURRENT_TIMESTAMP "
...
void Database::register_pending_button_removal(dpp::snowflake channel_id, dpp::snowflake message_id,
  ...
  execute([&](pqxx::connection& conn) {
    ...
    auto       expires_str =
        std::format("{:%Y-%m-%d %H:%M:%S}", std::chrono::floor<std::chrono::seconds>(expires_at));
    ...
    txn.exec_params(
        ...
        static_cast<int64_t>(channel_id), static_cast<int64_t>(message_id), expires_str);

    txn.commit();
    spdlog::debug("Registered pending button removal for message {} in channel {}",
                  message_id.str(), channel_id.str());
  });
}

std::vector<std::tuple<dpp::snowflake, dpp::snowflake, std::string>>
Database::get_expired_button_removals() {
  return execute([](pqxx::connection& conn) {
    pqxx::work txn(conn);

    auto       result =
        txn.exec("SELECT channel_id, message_id, removal_type FROM pending_button_removals "
...
std::vector<
    std::tuple<dpp::snowflake, dpp::snowflake, std::chrono::system_clock::time_point, std::string>>
...
bool Database::remove_pending_button_removal(dpp::snowflake channel_id, dpp::snowflake message_id) {
  return execute([&](pqxx::connection& conn) {
    ...
    auto       result = txn.exec_params(
        ...
        static_cast<int64_t>(channel_id), static_cast<int64_t>(message_id));

    txn.commit();

    if (result.affected_rows() > 0) {
      spdlog::debug("Removed pending button removal for message {} in channel {}", message_id.str(),
                    channel_id.str());
      return true;
    }
    return false;
...
void Database::set_embed_preset(dpp::snowflake discord_id, const std::string& preset) {
  execute([&](pqxx::connection& conn) {
    pqxx::work txn(conn);
    txn.exec_params("INSERT INTO user_settings (discord_id, embed_preset) VALUES ($1, $2) "
...
std::string Database::get_embed_preset(dpp::snowflake discord_id) {
  return execute([&](pqxx::connection& conn) -> std::string {
    pqxx::work txn(conn);
    auto result = txn.exec_params("SELECT embed_preset FROM user_settings WHERE discord_id = $1",
                                  static_cast<int64_t>(discord_id));
    if (result.empty())
      return "classic";
    return result[0][0].as<std::string>();
...
void Database::delete_embed_preset(dpp::snowflake discord_id) {
  execute([&](pqxx::connection& conn) {
    pqxx::work txn(conn);
    txn.exec_params("DELETE FROM user_settings WHERE discord_id = $1",
                    static_cast<int64_t>(discord_id));
    txn.commit();
  });
}
...
Database::get_user_custom_templates(dpp::snowflake discord_id) {
  return execute([&](pqxx::connection& conn) {
    pqxx::work txn(conn);
    auto       result = txn.exec_params(
...
std::optional<std::string> Database::get_user_custom_template(dpp::snowflake     discord_id,
...
void Database::set_user_custom_template(dpp::snowflake discord_id, const std::string& command_id,
                                        const std::string& json_config) {
  execute([&](pqxx::connection& conn) {
    pqxx::work txn(conn);
    txn.exec_params(
        "INSERT INTO user_custom_templates (discord_id, command_id, json_config) VALUES ($1, $2, "
        "$3) "
...
void Database::delete_user_custom_template(dpp::snowflake     discord_id,
                                           const std::string& command_id) {
  execute([&](pqxx::connection& conn) {
    pqxx::work txn(conn);
    txn.exec_params("DELETE FROM user_custom_templates WHERE discord_id = $1 AND command_id = $2",
                    static_cast<int64_t>(discord_id), command_id);
    txn.commit();
  });
}
...
void Database::delete_all_user_custom_templates(dpp::snowflake discord_id) {
  execute([&](pqxx::connection& conn) {
    pqxx::work txn(conn);
    txn.exec_params("DELETE FROM user_custom_templates WHERE discord_id = $1",
                    static_cast<int64_t>(discord_id));
    txn.commit();
  });
}
...
Database::get_all_preset_templates() {
  return execute([](pqxx::connection& conn) {
    pqxx::work txn(conn);
    ...
    for (const auto& row : result) {
      templates.emplace_back(row[0].as<std::string>(), row[1].as<std::string>(),
                             row[2].as<std::string>());
    }
    return templates;
  });
...
void Database::set_preset_template(const std::string& preset_name, const std::string& field_name,
                                   const std::string& template_text) {
  execute([&](pqxx::connection& conn) {
    pqxx::work txn(conn);
    txn.exec_params("INSERT INTO embed_preset_templates (preset_name, field_name, template) "
                    "VALUES ($1, $2, $3) "
...
void Database::delete_preset_templates(const std::string& preset_name) {
  execute([&](pqxx::connection& conn) {
    pqxx::work txn(conn);
    txn.exec_params("DELETE FROM embed_preset_templates WHERE preset_name = $1", preset_name);
    txn.commit();
  });
}
...
std::optional<std::string> Database::get_json_template(const std::string& key) {
  return execute([&](pqxx::connection& conn) -> std::optional<std::string> {
    pqxx::work txn(conn);
    auto       result = txn.exec_params(
        "SELECT json_config FROM embed_json_templates WHERE template_key = $1", key);
    if (result.empty()) {
      return std::nullopt;
    }
    return result[0][0].as<std::string>();
...
void Database::set_json_template(const std::string& key, const std::string& json_config) {
  execute([&](pqxx::connection& conn) {
    pqxx::work txn(conn);
    txn.exec_params("INSERT INTO embed_json_templates (template_key, json_config) "
                    "VALUES ($1, $2) "
...
void Database::delete_json_template(const std::string& key) {
  execute([&](pqxx::connection& conn) {
    pqxx::work txn(conn);
    txn.exec_params("DELETE FROM embed_json_templates WHERE template_key = $1", key);
    txn.commit();
  });
}
...
void Database::cache_discord_user(const DiscordUser& user) {
  execute([&](pqxx::connection& conn) {
    pqxx::work txn(conn);
    txn.exec_params("INSERT INTO discord_users (user_id, username, global_name, avatar_hash, "
                    "is_bot, updated_at) "
                    ...
                    "updated_at = CURRENT_TIMESTAMP",
                    static_cast<int64_t>(user.user_id), user.username,
...
std::optional<DiscordUser> Database::get_discord_user(dpp::snowflake user_id) {
  return execute([&](pqxx::connection& conn) -> std::optional<DiscordUser> {
    pqxx::work txn(conn);
    auto result = txn.exec_params("SELECT user_id, username, global_name, avatar_hash, is_bot FROM "
                                  "discord_users WHERE user_id = $1",
...
void Database::cache_guild_info(const GuildInfo& guild) {
  execute([&](pqxx::connection& conn) {
    pqxx::work txn(conn);
    txn.exec_params("INSERT INTO guild_info (guild_id, name, icon_hash, member_count, updated_at) "
                    ...
                    "updated_at = CURRENT_TIMESTAMP",
                    static_cast<int64_t>(guild.guild_id), guild.name,
...
void Database::cache_channel_info(const DiscordChannel& channel) {
  execute([&](pqxx::connection& conn) {
    ...
    txn.exec_params(
        ...
        "updated_at = CURRENT_TIMESTAMP",
        static_cast<int64_t>(channel.channel_id), static_cast<int64_t>(channel.guild_id),
...
std::optional<DiscordChannel> Database::get_channel_info(dpp::snowflake channel_id) {
  return execute([&](pqxx::connection& conn) -> std::optional<DiscordChannel> {
    pqxx::work txn(conn);
    auto result = txn.exec_params("SELECT channel_id, guild_id, channel_name, channel_type FROM "
                                  "discord_channels WHERE channel_id = $1",
                                  static_cast<int64_t>(channel_id));
...
void Database::log_download(int64_t beatmapset_id) {
  execute([&](pqxx::connection& conn) {
    pqxx::work txn(conn);
    txn.exec_params("INSERT INTO download_log (beatmapset_id) VALUES ($1)", beatmapset_id);
    txn.commit();
  });
}
...
void Database::cache_beatmap_id(int64_t beatmap_id, int64_t beatmapset_id,
  ...
  execute([&](pqxx::connection& conn) {
    ...
    txn.commit();
    spdlog::debug("[DB] Cached beatmap_id {} -> beatmapset_id {} (mode: {})", beatmap_id,
                  beatmapset_id, mode);
  });
}
...
std::optional<int64_t> Database::get_beatmapset_id(int64_t beatmap_id) {
  return execute([&](pqxx::connection& conn) -> std::optional<int64_t> {
    pqxx::work txn(conn);
    auto       result = txn.exec_params(
        "SELECT beatmapset_id FROM beatmap_id_cache WHERE beatmap_id = $1", beatmap_id);
...
void Database::register_osu_file(int64_t beatmap_id, int64_t beatmapset_id,
  ...
  execute([&](pqxx::connection& conn) {
    pqxx::work txn(conn);
    txn.exec_params("INSERT INTO osu_files (beatmap_id, beatmapset_id, file_path, file_size, "
    ...
    txn.commit();
    spdlog::debug("[DB] Registered .osu file for beatmap {} (set: {}, size: {} bytes)", beatmap_id,
                  beatmapset_id, file_size);
  });
}
...
bool Database::osu_file_exists(int64_t beatmap_id) {
  return execute([&](pqxx::connection& conn) {
    pqxx::work txn(conn);
    auto result = txn.exec_params("SELECT 1 FROM osu_files WHERE beatmap_id = $1", beatmap_id);
    return !result.empty();
  });
}
...
std::optional<std::string> Database::get_osu_file_path(int64_t beatmap_id) {
  return execute([&](pqxx::connection& conn) -> std::optional<std::string> {
    pqxx::work txn(conn);
    auto       result =
        txn.exec_params("SELECT file_path FROM osu_files WHERE beatmap_id = $1", beatmap_id);
...
void Database::store_message(const CrawledMessage& msg) {
  execute([&](pqxx::connection& conn) {
    ...
    txn.exec_params(
        "INSERT INTO discord_messages (message_id, channel_id, author_id, content, created_at, "
        "is_bot, "
        "reply_to_message_id, has_attachments, attachment_urls, edited_at) "
        "VALUES ($1, $2, $3, $4, $5::timestamp, $6, $7, $8, $9::text[], $10::timestamp) "
        "ON CONFLICT (message_id) DO NOTHING",
        static_cast<int64_t>(msg.message_id), static_cast<int64_t>(msg.channel_id),
        static_cast<int64_t>(msg.author_id), msg.content, created_str, msg.is_bot,
        msg.reply_to_message_id != 0 ?
            std::optional(static_cast<int64_t>(msg.reply_to_message_id)) :
            std::nullopt,
        msg.has_attachments, attachment_array, edited_str);
    txn.commit();
  });
}
...
bool Database::message_exists(dpp::snowflake message_id) {
  return execute([&](pqxx::connection& conn) {
    pqxx::work txn(conn);
    auto       result = txn.exec_params("SELECT 1 FROM discord_messages WHERE message_id = $1",
...
void Database::update_message(dpp::snowflake message_id, const std::string& new_content,
  ...
  execute([&](pqxx::connection& conn) {
    ...
    auto       edited_str =
        std::format("{:%Y-%m-%d %H:%M:%S}", std::chrono::floor<std::chrono::seconds>(edited_at));

    txn.exec_params("UPDATE discord_messages SET content = $1, edited_at = $2::timestamp "
                    "WHERE message_id = $3",
                    new_content, edited_str, static_cast<int64_t>(message_id));
    txn.commit();
...
void Database::process_all_messages(std::function<bool(const std::string&)> callback) {
  execute([&](pqxx::connection& conn) {
    pqxx::work txn(conn);
    auto       result =
        txn.exec("WITH deduplicated AS ("
                 "    SELECT content, created_at,"
                 "           LAG(created_at) OVER (PARTITION BY author_id, content ORDER BY "
                 "created_at) as prev_time"
    ...
    for (const auto& row : result) {
      if (!callback(row[0].as<std::string>()))
        break;
    }
  });
}

void Database::process_user_messages(dpp::snowflake                          author_id,
...
void Database::process_channel_messages(dpp::snowflake                          channel_id,
                                        std::function<bool(const std::string&)> callback) {
  execute([&](pqxx::connection& conn) {
    pqxx::work txn(conn);
    auto       result =
        txn.exec_params("WITH deduplicated AS ("
                        "    SELECT content, created_at,"
                        "           LAG(created_at) OVER (PARTITION BY author_id, content ORDER BY "
                        "created_at) as prev_time"
                        "    FROM discord_messages"
...
void Database::process_user_channel_messages(dpp::snowflake author_id, dpp::snowflake channel_id,
                                             std::function<bool(const std::string&)> callback) {
  execute([&](pqxx::connection& conn) {
    pqxx::work txn(conn);
    auto       result = txn.exec_params(
        "WITH deduplicated AS ("
              ...
              "SELECT content FROM deduplicated "
              "WHERE prev_time IS NULL OR created_at - prev_time > INTERVAL '30 seconds'",
        static_cast<int64_t>(author_id), static_cast<int64_t>(channel_id));
...
void Database::save_crawl_progress(const ChannelCrawlProgress& progress) {
  execute([&](pqxx::connection& conn) {
    ...
    auto       last_crawl_str = std::format(
        "{:%Y-%m-%d %H:%M:%S}", std::chrono::floor<std::chrono::seconds>(progress.last_crawl));
    ...
    txn.exec_params(
        ...
        static_cast<int64_t>(progress.channel_id), static_cast<int64_t>(progress.guild_id),
        progress.oldest_message_id != 0 ?
            std::optional(static_cast<int64_t>(progress.oldest_message_id)) :
            std::nullopt,
        progress.newest_message_id != 0 ?
            std::optional(static_cast<int64_t>(progress.newest_message_id)) :
            std::nullopt,
        static_cast<int64_t>(progress.total_messages), last_crawl_str,
        progress.initial_crawl_complete);
    txn.commit();
    spdlog::debug("[DB] Saved crawl progress for channel {}: {} messages", progress.channel_id,
...
std::optional<ChannelCrawlProgress> Database::get_crawl_progress(dpp::snowflake channel_id) {
  return execute([&](pqxx::connection& conn) -> std::optional<ChannelCrawlProgress> {
    pqxx::work txn(conn);
    auto       result =
        txn.exec_params("SELECT channel_id, guild_id, oldest_message_id, newest_message_id, "
                        "total_messages_crawled, last_crawl_at, initial_crawl_complete "
    ...
    progress.oldest_message_id =
        row[2].is_null() ? dpp::snowflake(0) : dpp::snowflake(row[2].as<uint64_t>());
    progress.newest_message_id =
        row[3].is_null() ? dpp::snowflake(0) : dpp::snowflake(row[3].as<uint64_t>());
...
std::vector<WordStatEntry> Database::get_top_words(size_t limit, const std::string& language,
                                                   bool exclude_stopwords) {
  return execute([&](pqxx::connection& conn) {
    pqxx::work   txn(conn);
    ...
    if (exclude_stopwords && !language.empty()) {
      result = txn.exec_params(base_query +
                                   "WHERE language = $1 AND word NOT IN (SELECT word FROM "
                                   "stopwords WHERE language = $1) " +
                                   order_limit + "$2",
                               language, static_cast<int64_t>(limit));
    } else if (exclude_stopwords) {
      result = txn.exec_params(base_query + "WHERE word NOT IN (SELECT word FROM stopwords) " +
                                   order_limit + "$1",
                               static_cast<int64_t>(limit));
    } else if (!language.empty()) {
      result = txn.exec_params(base_query + "WHERE language = $1 " + order_limit + "$2", language,
                               static_cast<int64_t>(limit));
    } else {
      result = txn.exec_params(base_query + order_limit + "$1", static_cast<int64_t>(limit));
    }
...
size_t Database::get_unique_word_count(const std::string& language, bool exclude_stopwords) {
  return execute([&](pqxx::connection& conn) {
    ...
    if (exclude_stopwords && !language.empty()) {
      ...
      result = txn.exec_params("SELECT COUNT(*) FROM word_stats WHERE language = $1", language);
    } else {
      result = txn.exec("SELECT COUNT(*) FROM word_stats");
    }
...
void Database::update_word_stats(
    const std::vector<std::tuple<std::string, size_t, std::string>>& words) {
  if (words.empty())
    return;
  ...
  execute([&](pqxx::connection& conn) {
    ...
    for (const auto& [word, count, language] : words) {
      txn.exec_params("INSERT INTO word_stats (word, count, language, last_updated) "
...
void Database::update_phrase_stats(
    const std::vector<std::tuple<std::string, std::vector<std::string>, int, size_t, std::string>>&
        phrases) {
  if (phrases.empty())
    return;
...
void Database::update_phrase_pmi_scores(
    const std::vector<std::tuple<std::string, std::string, double>>& phrase_pmi) {
  if (phrase_pmi.empty())
    return;
  ...
  execute([&](pqxx::connection& conn) {
    ...
    for (const auto& [phrase, language, pmi] : phrase_pmi) {
      txn.exec_params("UPDATE phrase_stats SET pmi_score = $1, last_updated = CURRENT_TIMESTAMP "
...
void Database::update_phrase_npmi_llr_scores(
  ...
  execute([&](pqxx::connection& conn) {
    ...
    for (const auto& [phrase, language, npmi, llr] : scores) {
      txn.exec_params("UPDATE phrase_stats SET npmi_score = $1, llr_score = $2, last_updated = "
                      "CURRENT_TIMESTAMP "
...
std::vector<PhraseStatEntry> Database::get_top_phrases(size_t limit, const std::string& language,
                                                       bool sort_by_pmi, int word_count_filter,
                                                       size_t min_count) {
  return execute([&](pqxx::connection& conn) {
    pqxx::work  txn(conn);

    std::string query =
        "SELECT phrase, words, word_count, count, pmi_score, npmi_score, llr_score, "
        "trend_score, first_seen, language "
        "FROM phrase_stats WHERE count >= " +
...
Database::get_global_phrase_frequencies(const std::string& language, int word_count_filter,
                                        size_t min_count) {
  return execute([&](pqxx::connection& conn) {
    pqxx::work  txn(conn);

    std::string query =
        "SELECT phrase, count FROM phrase_stats WHERE count >= " + std::to_string(min_count);
...
std::optional<std::chrono::system_clock::time_point> Database::get_last_phrase_snapshot_time() {
  return execute(
      [&](pqxx::connection& conn) -> std::optional<std::chrono::system_clock::time_point> {
        pqxx::work txn(conn);
...
std::unordered_map<std::string, size_t> Database::get_phrase_counts_from_days_ago(int days) {
  return execute([&](pqxx::connection& conn) {
    ...
    auto       result = txn.exec_params("SELECT phrase, count FROM phrase_history "
...
void Database::update_phrase_first_seen(
  ...
  execute([&](pqxx::connection& conn) {
    ...
    for (const auto& [phrase, language] : phrases) {
      txn.exec_params("UPDATE phrase_stats SET first_seen = CURRENT_TIMESTAMP "
...
int64_t Database::get_stats_last_message_id(const std::string& stats_key) {
  return execute([&](pqxx::connection& conn) -> int64_t {
    pqxx::work txn(conn);
    auto       result = txn.exec_params(
        "SELECT last_message_id FROM stats_processing_state WHERE key = $1", stats_key);
    if (result.empty()) {
      return 0;
    }
    return result[0][0].as<int64_t>();
...
void Database::set_stats_last_message_id(const std::string& stats_key, int64_t message_id) {
  execute([&](pqxx::connection& conn) {
    pqxx::work txn(conn);
    txn.exec_params("INSERT INTO stats_processing_state (key, last_message_id, last_processed_at) "
                    "VALUES ($1, $2, CURRENT_TIMESTAMP) "
                    "ON CONFLICT (key) DO UPDATE SET last_message_id = $2, last_processed_at = "
                    "CURRENT_TIMESTAMP",
...
int64_t
Database::process_messages_incremental(int64_t last_message_id,
...
void Database::upsert_word_stats(
    const std::vector<std::tuple<std::string, size_t, std::string>>& words) {
  if (words.empty())
    return;
  ...
  execute([&](pqxx::connection& conn) {
    ...
    for (const auto& [word, count, language] : words) {
      txn.exec_params("INSERT INTO word_stats (word, count, language) VALUES ($1, $2, $3) "
...
void Database::upsert_phrase_stats(
    const std::vector<std::tuple<std::string, std::vector<std::string>, int, size_t, std::string>>&
        phrases) {
  if (phrases.empty())
    return;
...
std::vector<Database::PhraseForPMI> Database::get_phrases_for_pmi(size_t min_count) {
  return execute([&](pqxx::connection& conn) {
    pqxx::work txn(conn);
    auto       result =
        txn.exec_params("SELECT phrase, words, count, language FROM phrase_stats WHERE count >= $1",
...
void Database::add_word_to_blacklist(const std::string& word, const std::string& language) {
  execute([&](pqxx::connection& conn) {
    pqxx::work txn(conn);
    txn.exec_params("INSERT INTO word_blacklist (word, language) VALUES ($1, $2) "
...
void Database::log_template_change(dpp::snowflake admin_id, const std::string& admin_username,
                                   const std::string& action, const std::string& command_id,
                                   const std::string& preset, const std::string& old_fields_json,
  ...
  execute([&](pqxx::connection& conn) {
    ...
    txn.exec_params(R"(
            ...
            VALUES ($1, $2, $3, $4, $5, $6::jsonb, $7::jsonb)
                    ...
                    static_cast<int64_t>(admin_id), admin_username, action, command_id,
    ...
    txn.commit();
    spdlog::info("[DB] Template audit: {} by {} ({}) - {}:{}", action, admin_username,
                 static_cast<uint64_t>(admin_id), command_id, preset);
  });
}
...
std::vector<TemplateAuditEntry> Database::get_template_audit_log_by_admin(dpp::snowflake admin_id,
                                                                          size_t         limit) {
  return execute([&](pqxx::connection& conn) {
    pqxx::work                      txn(conn);
    auto                            result = txn.exec_params(R"(
...
std::vector<TemplateAuditEntry>
Database::get_template_audit_log_by_command(const std::string& command_id, size_t limit) {
  return execute([&](pqxx::connection& conn) {
    pqxx::work                      txn(conn);
    auto                            result = txn.exec_params(R"(
            SELECT id, discord_id, discord_username, action, command_id, preset,
