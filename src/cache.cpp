...
MemcachedCache::MemcachedCache(const std::string& host, int port) {
  ...
  if (rc != MEMCACHED_SUCCESS) {
    ...
    throw std::runtime_error(
        std::format("Failed to add memcached server: {}", memcached_strerror(memc_, rc)));
  }
...
bool MemcachedCache::set(const std::string& key, const std::string& value,
                         std::chrono::seconds ttl) {
  std::lock_guard<std::mutex> lock(cache_mutex_);
  std::string                 full_key = build_key(key);
  memcached_return_t rc = memcached_set(memc_, full_key.c_str(), full_key.length(), value.c_str(),
                                        value.length(), static_cast<time_t>(ttl.count()), 0);

  if (rc != MEMCACHED_SUCCESS) {
    spdlog::warn("Memcached set failed for key '{}': {}", key, memcached_strerror(memc_, rc));
    return false;
  }
...
std::optional<std::string> MemcachedCache::get(const std::string& key) {
  ...
  char*                       value =
      memcached_get(memc_, full_key.c_str(), full_key.length(), &value_length, &flags, &rc);
  ...
  if (rc != MEMCACHED_SUCCESS) {
    spdlog::warn("Memcached get failed for key '{}': {}", key, memcached_strerror(memc_, rc));
    if (value)
      free(value);
    return std::nullopt;
  }
...
bool MemcachedCache::del(const std::string& key) {
  ...
  if (rc != MEMCACHED_SUCCESS && rc != MEMCACHED_NOTFOUND) {
    spdlog::warn("Memcached delete failed for key '{}': {}", key, memcached_strerror(memc_, rc));
    return false;
  }
...
std::optional<LeaderboardState> MemcachedCache::deserialize_leaderboard(const std::string& data) {
  try {
    ...
    state.sort_method = static_cast<LbSortMethod>(
        j.value("sort_method", 0)); // Default to PP for backward compatibility
    state.current_page      = j["current_page"];
    state.total_pages       = j["total_pages"];
    state.caller_discord_id = dpp::snowflake(
        j.value("caller_discord_id", 0ULL)); // Default to 0 for backward compatibility

    // Deserialize timestamp (defaults to now if not present for backwards compatibility)
    if (j.contains("created_at")) {
      auto millis      = j["created_at"].get<int64_t>();
      state.created_at = std::chrono::steady_clock::time_point(std::chrono::milliseconds(millis));
    }
    ...
    state.beatmap.status                   = static_cast<BeatmapStatus>(
        j["beatmap"].value("status", 0)); // Default to Pending for backwards compatibility
...
std::string MemcachedCache::serialize_recent_scores(const RecentScoreState& state) {
  ...
  for (const auto& [beatmap_id, data] : state.beatmap_difficulty_cache) {
    j["difficulty_cache"][std::to_string(beatmap_id)] = {std::get<0>(data), std::get<1>(data),
    ...
    if (j.contains("difficulty_cache")) {
      for (const auto& [key, value] : j["difficulty_cache"].items()) {
        ...
        if (value.size() >= 5) {
          ...
          state.beatmap_difficulty_cache[beatmap_id] =
              std::make_tuple(ar, od, cs, hp, total_objects);
        } else {
          // Old format: AR, OD, total_objects (use defaults for CS and HP)
          int total_objects = value.size() > 2 ? value[2].get<int>() : 0;
          state.beatmap_difficulty_cache[beatmap_id] =
              std::make_tuple(ar, od, 5.0f, 5.0f, total_objects);
        }
      }
    }
...
bool MemcachedCache::cache_recent_scores(const std::string&      state_id,
                                         const RecentScoreState& state) {
...
std::optional<std::pair<std::string, std::string>> MemcachedCache::get_oauth_tokens() {
  ...
  try {
    json j = json::parse(*data);
    return std::make_pair(j["access_token"].get<std::string>(),
...
bool MemcachedCache::set_top_role_users(const std::set<dpp::snowflake>& users) {
  nlohmann::json j = nlohmann::json::array();
  for (auto id : users) {
    j.push_back(static_cast<uint64_t>(id));
  }
  return set("top_role_users", j.dump(), std::chrono::seconds(300));
}

std::set<dpp::snowflake> MemcachedCache::get_top_role_users() {
  std::set<dpp::snowflake> result;
  auto                     val = get("top_role_users");
  if (!val)
    return result;
  try {
