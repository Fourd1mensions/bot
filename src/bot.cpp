...
// test
std::string page_content(const std::vector<std::string>& v, size_t page, size_t items_per_page = 5) {
    std::stringstream ss;
...
void Temp_data::add_temp(const dpp::snowflake& msg_id, const Beatmap& beatmap, std::vector<Score>& scores) {
    Data data(beatmap, scores);
    map[msg_id] = data;
    delayed_del(msg_id);
}

void Bot::write_cbid_map(const std::string& msg, const std::string& channel_id) {
...
void Bot::handle_button_click(const dpp::button_click_t& event) {
    ...
    auto back_button =  dpp::component().set_type(dpp::cot_button)
    ...
    dpp::message m = dpp::message(event.command.channel_id, "**Свойства молочка пачули**\n\n" + page_content(content, current_page, 1));
    m.add_component(dpp::component().add_component(back_button).add_component(next_button));
...
void Bot::create_lb_message(const dpp::message_create_t& event) {
    ...
    if (beatmap_it == channel_beatmapid_map.end()) {
        event.reply(dpp::message("Can't find the map. Please send the map link and use the command again."));
        return; 
    }
    ...
    std::mutex m;
    std::for_each(std::execution::par, disid_userid_map.begin(), disid_userid_map.end(), 
        [&](const auto& pair) {
    ...
    if (scores.size() > 1) {
        std::ranges::sort(scores, [](const Score a, const Score b) {
            return std::make_tuple(a.get_pp(), a.get_total_score()) > std::make_tuple(b.get_pp(), b.get_total_score());
        });
    }
...
void Bot::handle_slashcommand(const dpp::slashcommand_t& event) {
    ...
    if (event.command.get_command_name() == "update_token") {
        ...
        if (request.set_tokens("refresh_token")) event.reply(dpp::message("Token update - success"));
        else event.reply(dpp::message("Token update - fail"));
    }
    ...
    if (event.command.get_command_name() == "score") {
        ...
        user = user_it->second;
        std::string beatmap_id;
        auto beatmap_it = channel_beatmapid_map.find(event.command.channel_id.str());
        if (beatmap_it == channel_beatmapid_map.end()) {
            event.reply(dpp::message("Can't find the map. Please send the map link and use the command again."));
            return; 
        }
...
void Bot::ready_event(const dpp::ready_t& event) {
    if (dpp::run_once<struct register_bot_commands>()) {
        ...
        bot.global_command_create(dpp::slashcommand("patchouli", "Information", bot.me.id));
        ...
        bot.global_command_create(dpp::slashcommand("score", "Displays your score", bot.me.id));
