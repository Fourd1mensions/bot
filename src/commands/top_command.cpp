...
bool TopCommand::matches(const CommandContext& ctx) const {
  ...
  if (ctx.content_lower.find(p + "top") == 0 && check_boundary(ctx.content_lower, p.size() + 3))
    return true;
  // Cyrillic: tolower doesn't work with UTF-8, check both cases
  std::string cyr_lo = p + "ещз", cyr_up = p + "ЕЩЗ";
  if ((ctx.content.find(cyr_lo) == 0 || ctx.content.find(cyr_up) == 0) &&
      check_boundary(ctx.content, cyr_lo.size()))
    return true;
  return false;
}
  ...
  if (colon_pos != std::string::npos && colon_pos < cmd_end) {
    ...
    if (result.mode != "osu" && result.mode != "taiko" && result.mode != "catch" &&
      ...
      result.error_message =
          "Invalid mode. Supported modes: `osu`, `taiko`, `catch`/`fruits`, `mania`";
      return result;
    }
  ...
  for (size_t i = 0; i < tokens.size(); ++i) {
    ...
    if (tok == "-s" || tok == "--sort") {
      if (i + 1 < tokens.size()) {
        ...
        if (sort_str == "pp")
          result.sort_method = TopSortMethod::PP;
        else if (sort_str == "acc" || sort_str == "accuracy")
          result.sort_method = TopSortMethod::Acc;
        else if (sort_str == "score")
          result.sort_method = TopSortMethod::Score;
        else if (sort_str == "combo")
          result.sort_method = TopSortMethod::Combo;
        else if (sort_str == "date" || sort_str == "recent")
          result.sort_method = TopSortMethod::Date;
        else if (sort_str == "miss" || sort_str == "misses")
          result.sort_method = TopSortMethod::Misses;
        else {
          result.valid = false;
          result.error_message =
              "Invalid sort method. Use: `pp`, `acc`, `score`, `combo`, `date`, `misses`";
          return result;
        }
      }
      continue;
...
void TopCommand::execute_unified(const UnifiedContext& ctx) {
  ...
  if (ctx.is_slash()) {
    ...
    if (auto sort = ctx.get_string_param("sort")) {
      ...
      if (sort_str == "pp")
        params.sort_method = TopSortMethod::PP;
      else if (sort_str == "acc")
        params.sort_method = TopSortMethod::Acc;
      else if (sort_str == "score")
        params.sort_method = TopSortMethod::Score;
      else if (sort_str == "combo")
        params.sort_method = TopSortMethod::Combo;
      else if (sort_str == "date")
        params.sort_method = TopSortMethod::Date;
      else if (sort_str == "misses")
        params.sort_method = TopSortMethod::Misses;
    }
    ...
    if (params.username.empty()) {
      ...
      auto embed =
          dpp::embed()
              ...
              .set_description(
                  "Link your osu! account to use commands without specifying your username.")
              .add_field("Option 1: Website",
                         fmt::format("[Open Settings]({})", s->config.public_url + "/osu/settings"),
                         true)
              .add_field("Option 2: Direct Link", "Click the button below to get a link in DMs",
                         true)
  ...
  std::string scores_response =
      s->request.get_user_best_scores(std::to_string(osu_user_id), params.mode, 100, 0);
    ...
    scores.erase(std::remove_if(scores.begin(), scores.end(),
                                [&](const Score& score) {
                                  ...
                                  for (const auto& mod : required_mods) {
                                    bool found = utils::has_mod(score_mods, mod);
                                    if (exclude && found)
                                      return true; // Exclude if has mod
                                    if (!exclude && !found)
                                      return true; // Include only if has mod
                                  }
                                  return false;
                                }),
                 scores.end());
  ...
  auto compare = [&params](const Score& a, const Score& b) {
    ...
    switch (params.sort_method) {
      case TopSortMethod::Acc: result = a.get_accuracy() > b.get_accuracy(); break;
      case TopSortMethod::Score: result = a.get_total_score() > b.get_total_score(); break;
      case TopSortMethod::Combo: result = a.get_max_combo() > b.get_max_combo(); break;
      case TopSortMethod::Date: result = a.get_created_at() > b.get_created_at(); break;
      case TopSortMethod::Misses:
        result = a.get_count_miss() < b.get_count_miss(); // Less misses = better
        break;
      case TopSortMethod::PP:
      default: result = a.get_pp() > b.get_pp(); break;
    }
  ...
  if (scores.empty()) {
    ...
    if (!params.mods_filter.empty())
      filter_msg += fmt::format(" mods={}", params.mods_filter);
    if (!params.grade_filter.empty())
      filter_msg += fmt::format(" grade={}", params.grade_filter);
  ...
  if (params.index > 0) {
    if (params.index > scores.size()) {
      ctx.reply(s->message_presenter.build_error_message(fmt::format(
          "Score #{} not found (only {} scores after filtering)", params.index, scores.size())));
      return;
    }
  ...
  TopState top_state(std::move(scores), resolved_username, osu_user_id, params.mode,
                     params.mods_filter, params.grade_filter, params.sort_method, params.reverse,
                     ctx.author_id(), preset);
  ...
  if (suggest_oauth_link) {
    std::string hint = fmt::format(
        "-# Link your osu! account with `{}link` or `/link` for a better experience", ctx.prefix);
  ...
  ctx.reply(
      std::move(msg), false,
      [s, top_state = std::move(top_state)](const dpp::confirmation_callback_t& callback) mutable {
