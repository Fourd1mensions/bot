...
SimCommand::ParsedParams SimCommand::parse(const std::string& content,
  ...
  std::string content_lower_copy = content;
  std::transform(content_lower_copy.begin(), content_lower_copy.end(), content_lower_copy.begin(),
                 [](unsigned char c) { return std::tolower(c); });

  size_t pp_pos      = content_lower_copy.find("pp");
  size_t percent_pos = content.find('%');

  if (pp_pos != std::string::npos && pp_pos > 0 &&
      (percent_pos == std::string::npos || pp_pos < percent_pos)) {
    size_t num_end   = pp_pos;
    size_t num_start = content.find_last_not_of("0123456789.", num_end - 1);
    num_start        = (num_start == std::string::npos) ? 0 : num_start + 1;

    if (num_start < num_end) {
      std::string pp_str = content.substr(num_start, num_end - num_start);
      try {
        result.target_pp = std::stod(pp_str);
        if (result.target_pp <= 0.0) {
          result.valid         = false;
          result.error_message = "Target PP must be positive.";
          return result;
        }
      } catch (...) {
        result.valid         = false;
        result.error_message = fmt::format("Invalid PP value. Example: `{}sim 700pp`", prefix);
        return result;
      }
    }
  }

  if (result.target_pp > 0.0 && percent_pos != std::string::npos) {
    result.valid = false;
    result.error_message =
        fmt::format("Use either `{}sim 99%` or `{}sim 700pp`, not both.", prefix, prefix);
    return result;
  }
  ...
  if (result.target_pp <= 0.0) {
    if (percent_pos == std::string::npos) {
      ...
      result.error_message = fmt::format(
          "Usage: `{}sim[:mode] <accuracy>% [+mods] [...]` or `{}sim[:mode] <pp>pp [+mods] [...]`\n"
          "Examples:\n"
          "• `{}sim 99% +HDDT` - simulate 99%% accuracy\n"
          "• `{}sim 700pp +HDDT` - find accuracy needed for 700pp",
  ...
  if (plus_pos != std::string::npos) {
    ...
    result.mods_filter.erase(std::remove(result.mods_filter.begin(), result.mods_filter.end(), ' '),
                             result.mods_filter.end());
    result.mods_filter.erase(std::remove(result.mods_filter.begin(), result.mods_filter.end(), '+'),
                             result.mods_filter.end());
...
void SimCommand::execute_unified(const UnifiedContext& ctx) {
  ...
  if (ctx.is_slash()) {
    if (auto pp = ctx.get_double_param("pp")) {
      parsed.target_pp = *pp;
    } else if (auto accuracy = ctx.get_double_param("accuracy")) {
      parsed.accuracy = *accuracy / 100.0;
    } else {
      ctx.reply("Either accuracy or pp is required.");
      return;
    }
  ...
  if (parsed.target_pp > 0.0) {
    ...
    for (int i = 0; i < 50; ++i) {
      ...
      params.accuracy = mid;
      auto r = s->performance_service.calculate_pp(beatmapset_id, beatmap_id, parsed.mode, params);
      if (!r)
        break;

      double diff = r->pp - parsed.target_pp;
    ...
    if (!best_result) {
      ctx.reply(s->message_presenter.build_error_message("Failed to find accuracy for target PP."));
      return;
    }

    if (std::abs(best_diff) > parsed.target_pp * 0.1 && std::abs(best_diff) > 10.0) {
      ctx.reply(s->message_presenter.build_error_message(
          fmt::format("Target {:.0f}pp is not achievable on this map with these settings. "
                      "Closest: {:.0f}pp at {:.2f}% acc.",
                      parsed.target_pp, best_result->pp, best_acc * 100)));
      return;
    }

    parsed.accuracy = best_acc;
    result          = best_result;
  } else {
    params.accuracy = parsed.accuracy;
    result = s->performance_service.calculate_pp(beatmapset_id, beatmap_id, parsed.mode, params);
  }

  if (!result.has_value()) {
    ctx.reply(s->message_presenter.build_error_message("Failed to simulate score."));
    return;
  }
