...
void AdminSetCommand::execute_unified(const UnifiedContext& ctx) {
  ...
  bool        is_admin  = std::find(s->config.admin_users.begin(), s->config.admin_users.end(),
                                    caller_id) != s->config.admin_users.end();
  ...
  if (args.empty()) {
    ctx.reply(
        fmt::format(":x: Usage: `{}adminset <discord_id_or_mention> <osu_username>`", ctx.prefix));
    return;
  }
  ...
  try {
    ...
    spdlog::info("[adminset] Admin {} linked Discord {} to osu! {} ({})", caller_id,
                 target_discord_id.str(), osu_user_id, resolved_username);
