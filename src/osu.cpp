...
std::string Score::to_string(const uint32_t beatmap_combo) const {
    ...
    std::string emoji_id;
    if (rank == "F") emoji_id = "<:RankingF:1278036373332295843>";
    else if (rank == "D") emoji_id = "<:RankingD:1278036354248474674>";
    else if (rank == "C") emoji_id = "<:RankingC:1278036342441250998>";
    else if (rank == "B") emoji_id = "<:RankingB:1278036331099852810>";
    else if (rank == "A") emoji_id = "<:RankingA:1278036315421671424>";
    else if (rank == "S") emoji_id = "<:RankingS:1278036387433680968>";
    else if (rank == "SH") emoji_id = "<:RankingSH:1278036405230108744>";
    else if (rank == "X") emoji_id = "<:RankingSS:1304449505873367130>";
    else if (rank == "XH") emoji_id = "<:RankingSSH:1304449533006057544>";
    
    std::string result = fmt::format(
        "**{}  •  {:.2f}pp  •  {} ({:.2f}%)  +{}**\n{:L}  **•**  x{}/{}  **•**  [{}/{}/{}/{}]", 
        username, pp, emoji_id, accuracy * 100, mods, 
        total_score, max_combo, beatmap_combo, 
...
std::string Beatmap::to_string () const {
    return fmt::format(
        "{} - {} [{}] {}★",
        artist, title, version, difficulty_rating
    );
}
