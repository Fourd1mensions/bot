...
private:
  ...
  std::chrono::seconds                          alert_cooldown_{300};
  std::chrono::steady_clock::time_point         last_alert_{};
...
public:
  ...
  static void init(const std::string& host, int port, const std::string& dbname,
                   const std::string& user, const std::string& password, size_t pool_size = 5);

  // Get the singleton instance
  static Database& instance();

  void             set_alert_callback(ConnectionPool::AlertCallback cb,
                                      std::chrono::seconds          cooldown = std::chrono::seconds(300));
  ...
  void register_osu_file(int64_t beatmap_id, int64_t beatmapset_id, const std::string& file_path,
                         int64_t file_size = 0);
  ...
  std::optional<std::string> get_user_custom_template(dpp::snowflake     discord_id,
                                                      const std::string& command_id);

  /// Set/update a user's custom template for a command
  void set_user_custom_template(dpp::snowflake discord_id, const std::string& command_id,
  ...
  void set_preset_template(const std::string& preset_name, const std::string& field_name,
  ...
  void process_user_messages(dpp::snowflake                          author_id,
                             std::function<bool(const std::string&)> callback);

  // Get all message contents for a specific channel (streaming)
  void process_channel_messages(dpp::snowflake                          channel_id,
                                std::function<bool(const std::string&)> callback);

  // Get all message contents for a specific user in a specific channel (streaming)
  void process_user_channel_messages(dpp::snowflake author_id, dpp::snowflake channel_id,
                                     std::function<bool(const std::string&)> callback);
  ...
  std::vector<WordStatEntry> get_top_words(size_t limit, const std::string& language = "",
                                           bool exclude_stopwords = false);
  ...
  void update_phrase_stats(const std::vector<std::tuple<std::string,              // phrase
                                                        std::vector<std::string>, // words
                                                        int,                      // word_count
                                                        size_t,                   // count
                                                        std::string               // language
  ...
  void update_phrase_npmi_llr_scores(
      const std::vector<std::tuple<std::string, std::string, double, double>>&
          scores // phrase, language, npmi, llr
  );

  // Get top phrases (sorted by count or PMI)
  std::vector<PhraseStatEntry> get_top_phrases(size_t limit, const std::string& language = "",
                                               bool sort_by_pmi = false, int word_count_filter = 0,
                                               size_t min_count = 5);
  ...
  get_global_phrase_frequencies(const std::string& language = "", int word_count_filter = 0,
                                size_t min_count = 5);
  ...
  void save_phrase_history_snapshot(const std::vector<std::tuple<std::string, std::string, size_t>>&
                                        phrases // phrase, language, count
  );
  ...
  void update_phrase_trend_scores(const std::vector<std::tuple<std::string, std::string, double>>&
                                      trends // phrase, language, trend_score
  );
  ...
  int64_t process_messages_incremental(int64_t last_message_id,
  ...
  void upsert_phrase_stats(const std::vector<std::tuple<std::string,              // phrase
                                                        std::vector<std::string>, // words
                                                        int,                      // word_count
                                                        size_t,                   // count
                                                        std::string               // language
  ...
  void log_template_change(
      dpp::snowflake admin_id, const std::string& admin_username,
  ...
  std::vector<TemplateAuditEntry> get_template_audit_log(size_t limit = 50, size_t offset = 0);

  // Get audit log entries for a specific admin
  std::vector<TemplateAuditEntry> get_template_audit_log_by_admin(dpp::snowflake admin_id,
                                                                  size_t         limit = 50);

  // Get audit log entries for a specific command
  std::vector<TemplateAuditEntry> get_template_audit_log_by_command(const std::string& command_id,
                                                                    size_t             limit = 50);
