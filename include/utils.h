...
struct Config {
  std::string api_v1_key, client_id, client_secret, auth_code, access_token, refresh_token,
      redirect_uri, weather_api_key;
...
bool map_to_file(const std::unordered_map<K, V>& map, const std::string& path) {
  static_assert((std::is_same_v<K, std::string> || std::is_same_v<K, dpp::snowflake>) &&
...
struct ModsValidationResult {
  ...
  bool                     is_valid() const {
    return invalid.empty() && !has_incompatible;
  }
};

// Valid osu! mod codes
inline const std::vector<std::string> VALID_MODS = {
    "NF", "EZ", "TD", "HD", "HR", "SD", "DT", "RX", "HT", "NC", "FL", "AT", "SO", "AP",  "PF", "K4",
    "K5", "K6", "K7", "K8", "FI", "RN", "CN", "TP", "K9", "KC", "K1", "K3", "K2", "SV2", "MR"};
...
// Check if a score's mods string contains a required mod,
// treating equivalent mod pairs as interchangeable (DT↔NC, SD↔PF)
inline bool has_mod(const std::string& score_mods, const std::string& mod) {
  if (score_mods.find(mod) != std::string::npos)
    return true;
  if (mod == "DT")
    return score_mods.find("NC") != std::string::npos;
  if (mod == "NC")
    return score_mods.find("DT") != std::string::npos;
  if (mod == "SD")
    return score_mods.find("PF") != std::string::npos;
  if (mod == "PF")
    return score_mods.find("SD") != std::string::npos;
  return false;
}
