...
struct LeaderboardState {
  ...
  LeaderboardState() : mode("osu"), current_page(0), total_pages(0), created_at(std::chrono::steady_clock::now()), caller_discord_id(0) {}
  LeaderboardState(std::vector<Score> s, Beatmap b, size_t page = 0, std::string m = "osu", std::string mods = "", dpp::snowflake caller_id = 0)
    : scores(std::move(s)), beatmap(std::move(b)), mode(std::move(m)), mods_filter(std::move(mods)), current_page(page),
